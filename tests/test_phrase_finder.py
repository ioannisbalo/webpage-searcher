...
class TestPhraseFinder(TestCase):
    ...
    def test_find_phrase(self):
        cases = [
            {"phrase": "Chili Piper Get", "results": [
            ...
            {"phrase": "brands reach their marketing goals and beyond", "results": [
    ...
    def test_find_phrase_in_anchor(self):
        ...
        expected = [Result(tag='a', xpath='/html/body/p/a', string='google', link=Link(href='https://google.com', nofollow=False))]
        self._assert_case(html, "google", expected)

        no_follow_html = "<html><body><p>this is <a href=\"https://google.com\" rel=\"nofollow\">google</a>!</p></body></html>"
        expected[0].link.nofollow = True
        self._assert_case(no_follow_html, "google", expected)

    def test_find_multiple(self):
        html = "<html><body><div>one<p> two </p><a href=\"sample.com\">one</a></div><p>one <span>two</span></p></body></html>"
        expected = [
            Result(tag='div', xpath='/html/body/div', string='one two one'),
            Result(tag='p', xpath='/html/body/p', string='one two'),
            Result(tag='a', xpath='/html/body/div/a', string='one', link=Link(href='sample.com', nofollow=False)),
        ]
